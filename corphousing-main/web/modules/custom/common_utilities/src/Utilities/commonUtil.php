...
class commonUtil {
    ...
    public static function checkVendor($node) {
        $arr_admin_roles = ['site_admin', 'administrator'];
        $user = User::load(\Drupal::currentUser()->id());
        $roles = $user->getRoles();

        $vendor_id = $user->hasField('field_vendor_id') ? $user->get('field_vendor_id')->value : NULL;
        ...
        if (!empty($vendor_id)) {
            ...
            if ($node->hasField('field_vendor_id') && $node->get('field_vendor_id')->value == $vendor_id) {
                return true;
            }
        }
    ...
    public static function validateVendor() {
        ...
        if (!array_intersect($roles, $arr_vendor_roles) || empty($vendor_id)) {
            \Drupal::messenger()->addError("Sorry, You do not have permission to access this section.");
    ...
    public static function generate_property_id($property_name, $area_name) {
        return hash('sha256', $property_name . '|' . $area_name); 
    }
