...
const HOME_SCREEN_ANIMATION_DURATION_IN_SECONDS = 1.25;
const LOG_AND_NAVIGATION_ANIMATION_DELAY_IN_SECONDS = 0.5;
...
var e = {
  ...
  // START: 01 Preloader
  preLoader: async function () {
    window.onload = async function () {
      var preloader = e.select(".preloader");
      if (e.isVariableDefined(preloader)) {
        preloader.className += " animate__animated animate__fadeOut";
        preloader.style.transition = `height ${HOME_SCREEN_ANIMATION_DURATION_IN_SECONDS}s ease-in-out`;
        preloader.style.height = "0";
  
        var element = document.querySelector(".spinner-grow"); 
        // element.style.position = 'fixed';
  
        setTimeout(() => {
          // e.animateLogoAndNavigation();

          preloader.style.display = 'none';
          e.setAOSAnimtions();
        }, HOME_SCREEN_ANIMATION_DURATION_IN_SECONDS * 1000);
      }
    };
  },  
  // END: Preloader
  ...
  // START: 09 GLightbox
  lightBox: function () {
    const viewAllBtn = document.querySelector('.icon-gallery');
  
    var light = e.select("[data-glightbox]");
    if (e.isVariableDefined(light)) {
      e.lightBoxInstance = GLightbox({
      ...
      if (viewAllBtn) {
        viewAllBtn.addEventListener('click', () => {
          try {
            const originalOrder = Array.from(e.lightBoxInstance.elements);
            e.lightBoxInstance.elements = originalOrder.reverse();  
          } catch (error) {
            console.log('Error reversing images order');
          }
        });
      }
    }
