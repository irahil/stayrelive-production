                ...
                if ($record['action'] == 'insert' || $record['action'] == 'update') {
                    foreach ($arr_property['field_mapping'] as $key_node_field_name => $val_json_field_details) {
                        if (isset($val_json_field_details['direct'])) {
                            ...
                            $arr_node_load_data[$key_node_field_name] = (isset($record['data'][$level1][$level2])) ? $record['data'][$level1][$level2] : '';
                        } else if (isset($val_json_field_details['serialize'])) {
                            $arr_node_load_data[$key_node_field_name] = (isset($record['data'][$val_json_field_details['serialize'][0]])) ? serialize($record['data'][$val_json_field_details['serialize'][0]]) : '';
                        } else if (isset($val_json_field_details['taxonomy'])) {
                            $arr_terms = array();

                            if (isset($record['data'][$val_json_field_details['taxonomy'][0]])) {
                                $arr_terms = Importer::getTaxonomyTerms($record['data'][$val_json_field_details['taxonomy'][0]], $val_json_field_details['machine_name']);
                            }
